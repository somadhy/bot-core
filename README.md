...
- **`flood_ack`**: после успешной отправки в канал companion возвращает `MSG_SENT` с признаком маршрута (**flood** = `type == 1`) или **`PACKET_OK`** без флага. Для **flood** (и для OK-only) бот **`interval_sec`** секунд слушает **два** источника: **`RX_LOG_DATA`** (сырой RF-лог `0x88`, если прошивка шлёт) и **`CHANNEL_MSG_RECV`** — обычная очередь принятых строк канала с USB (то, что отдаёт `get_msg`); на многих сборках повторы видны только так. Совпадение по **каналу** и **тексту** (с допусками); время используется мягко, чтобы не отсечь рассинхрон часов. В лог пишется **путь** и **последний хоп**, если они есть (часто путь появляется только если в памяти узла уже был пакет из RF-лога). **`interval_sec: 0`** — не слушать. **`max_attempts`**: повторная **отправка**, если за интервал не было совпадений. Для **direct** (`type == 0`) прослушивание не выполняется. Рекомендуется **`set_decrypt_channel_logs(True)`** и загрузка слотов канала (`get_channel`).
