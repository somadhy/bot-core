...
| `мсг` / `msg` `индекс:текст` | Отправка в канал от лица бота (тот же протокол, что и `send_chan_msg`, с ожиданием ACK для flood — см. **`flood_ack`** в конфиге); длина текста как у обычных ответов бота в канале (~`MAX_MESSAGE_LEN` в коде). **Только личка**, **только админ**; индекс должен быть в `channels.enabled_indices`. **Первое** `:` в аргументе отделяет индекс от текста (остальные `:` — часть сообщения). Подтверждение или ошибка в ЛС отправляются с учётом **`dm_delivery`**. В каналах не обрабатывается. |
...
- **`flood_ack`** (надёжность **канальных** сообщений после отправки через Companion): прошивка возвращает `MSG_SENT` с признаком маршрута; для **flood** (`type == 1` в ответе) бот ждёт **ACK** с ожидаемым кодом из ответа — по смыслу подтверждение, что пакет **услышали** (в т.ч. репитеры). Если за **`flood_ack.interval_sec`** секунд ACK нет, сообщение **отправляется снова**, не более **`flood_ack.max_attempts`** попыток подряд (по умолчанию 5 с и 3 попытки). Для **direct**-канала ACK при необходимости ждётся один раз, **без** повторной отправки при таймауте. Если **`interval_sec: 0`**, ожидание ACK для канала отключено (после `MSG_SENT` повторы не делаются). Бот **ждёт только `MSG_SENT`** на отправку в канал (не смешивает с `PACKET_OK`): у части прошивок сначала приходит быстрый **`PACKET_OK`**, затем **`MSG_SENT`** с `expected_ack`; библиотека meshcore при ожидании «первого из OK и MSG_SENT» отдавала **только OK** — без `expected_ack`, хотя по эфиру и в приложениях всё было с heard/ACK. Сейчас OK на этом шаге игнорируется, пока не придёт `MSG_SENT`. Если узел шлёт **только** `PACKET_OK` без `MSG_SENT`, отправка в канал завершится ошибкой таймаута (такой режим не поддерживается).
